...
describe "Board" do
  ...
  describe "#get_empty_spaces" do
    it "returns array of available spaces" do
      expect(board1.get_empty_spaces).to eq(["1", "2", "3", "4", "5", "6", "7", "8", "9"])
    end

    it "returns array of empty spaces" do
      expect(board2.get_empty_spaces).to eq(["2", "4", "6", "8", "9"])
    end

    it "returns array of empty spaces" do
      expect(tie_board.get_empty_spaces).to eq([])
    end
  end

  describe "#get_empty_corner_spaces" do
    it "returns array of empty corner spaces" do
      expect(board1.get_empty_corner_spaces).to eq(["1", "3", "7", "9"])
    end

    it "returns array of empty corner spaces" do
      expect(board2.get_empty_corner_spaces).to eq(["9"])
    end

    it "returns array of empty corner spaces" do
      expect(tie_board.get_empty_corner_spaces).to eq([])
    end
  end

  describe "#get_empty_side_spaces" do
    it "returns array of empty side spaces" do
      expect(board1.get_empty_side_spaces).to eq(["2", "4", "6", "8"])
    end

    it "returns array of empty side spaces" do
      expect(won_board.get_empty_side_spaces).to eq(["4", "6", "8"])
    end

    it "returns array of empty side spaces" do
      expect(tie_board.get_empty_side_spaces).to eq([])
    end
  end
  ...
  describe "#check_for_immediate_move" do
    it "Returns an available space on the board that is a winning move" do
      expect(board2.check_for_immediate_move("O")).to eq 1
    end

    it "Returns nil if there is no winning move" do
      expect(board1.check_for_immediate_move("O")).to eq nil
    end
  end
